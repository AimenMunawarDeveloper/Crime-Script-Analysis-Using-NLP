...
Handles training Doc2Vec models, generating embeddings, and computing similarity matrices
"""
...
class Doc2VecModel:
    """Class for training and using Doc2Vec models"""
    ...
    def create_tagged_documents(self, texts: List[str], ids: List = None) -> List[TaggedDocument]:
        ...
        Args:
            ...
            ids: List of document IDs (optional, uses index if not provided)
    ...
    def train(self, corpus: List[TaggedDocument], verbose: bool = True):
        """
        Train Doc2Vec model on the provided corpus
        
        Args:
            corpus: List of TaggedDocument objects
    ...
    def generate_embeddings(self, texts: List[str] = None) -> np.ndarray:
        ...
        Args:
            texts: List of preprocessed text strings. If None, uses training corpus
        
        Returns:
            Numpy array of embeddings (n_documents x vector_size)
        """
    ...
    def compute_similarity_matrix(self, embeddings: np.ndarray = None) -> np.ndarray:
        """
        Compute cosine similarity matrix for document embeddings
        
        Args:
            embeddings: Numpy array of embeddings. If None, uses training corpus embeddings
    ...
    def get_most_similar_documents(self, doc_id: int, topn: int = 10) -> List[Tuple[int, float]]:
        """
        ...
        similar_docs = self.model.dv.most_similar([doc_vector], topn=topn+1)
        
        return [(sim_doc_id, score) for sim_doc_id, score in similar_docs if sim_doc_id != doc_id][:topn]
