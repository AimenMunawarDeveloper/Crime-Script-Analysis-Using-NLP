...
If you get an error that `pip` is not recognized, use `python -m pip` instead:

```bash
python -m pip install -r requirements.txt
```

This ensures you're using the pip associated with your Python installation.

### ".NET" or compilation errors

If you encounter errors about ".NET" or compilation when installing packages:

1. Make sure you're using pre-built wheels (pip should do this automatically)
2. Upgrade pip: `python -m pip install --upgrade pip`
3. Try installing packages individually if the requirements file fails

## Notes

- The preprocessing pipeline is based on the original implementation but has been refactored into a modular structure
- The Doc2Vec implementation focuses on generating embeddings and similarity matrices
- All output files are saved in CSV format for easy analysis
